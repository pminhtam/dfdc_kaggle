...
running_loss = 0
print_every = 1
checkpoint = "./resnetx_kaggle_pytorch/"
...
model.load_state_dict(torch.load("/home/tampm/code/kaggle/resnext.pth"))
...
with torch.no_grad():
    for inputs, labels in testloader:
        print(labels)
        inputs, labels = inputs.to(device),(1-labels).float().to(device)
        print(labels)
        exit(0)
...
#                 train_losses.append(running_loss/len(trainloader))
    #             test_losses.append(test_loss/len(testloader))                    
            ...
            running_loss = 0
            model.train()
    torch.save(model.state_dict(), os.path.join(checkpoint, 'resnetx_pytorch_%d.pt' % epoch))
