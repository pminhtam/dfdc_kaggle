...
#         '/data/tam/kaggle/test_imgs/',
    '/hdd/tam/kaggle/extract_raw_img_test/',
        target_size=(IMAGE_SHAPE, IMAGE_SHAPE),
        batch_size=batch_size,
        class_mode='binary',
        subset='training')
...
# model.fit_generator(generator,steps_per_epoch=len(generator),workers=16)


from focal_loss import BinaryFocalLoss

model.load_weights("log_xception_keras_focalloss/checkpoint_newdata_0003.pth")
...
checkpoints = keras.callbacks.ModelCheckpoint("./log_xception_keras_focalloss/checkpoint_newdata2_{epoch:04d}.pth", monitor='val_loss', verbose=0, save_best_only=False, period=1)

early_stop = keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0, patience=0)

model.fit_generator(generator,validation_data=test_generator,steps_per_epoch=len(generator), epochs=10,workers=workers,validation_steps=14298/batch_size,class_weight = class_weights_2,callbacks = [tensorboard_callback,checkpoints])
